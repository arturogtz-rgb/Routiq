...
- **Recordatorio de pago = acción MANUAL del ejecutivo:** nuevo `POST /api/ai/quotations/{id}/follow-up-payment` (kind 'payment' en `ai_service.follow_up_message`, agrega saldo + enlace de pago). Botón `ai-followup-payment-btn` en el Asistente de seguimiento, visible solo si `state='ganada' && payment_status!='paid'`. El ejecutivo redacta/edita y envía por WhatsApp/correo. Nada se envía solo.
- **Digest interno OPT-IN (nunca al cliente):** `companies.internal_payment_digest` (default False, toggle en Ajustes→Pagos `internal-digest-checkbox`). `run_internal_payment_digest` agrupa por ejecutivo (`created_by`), envía solo al ejecutivo/`notify_email`, idempotente por empresa por día vía guard atómico `find_one_and_update(last_internal_digest_date != today)` ANTES de enviar → imposible el doble envío.
- **Punto 1 — Programa personalizado (CustomQuotationBuilder):** agregados selectores de tipo (`type-paquete`→/new, `type-servicios`→/new?type=servicios, `type-personalizado` activo), buscador `custom-client-search-input`, y nuevo paso `executive` (`custom-step-executive-panel`) para clientes con ejecutivos (se omite en cliente directo). QuotationBuilder ahora lee `?type=servicios`.
- **Punto 2 — Notas internas en línea:** `PATCH /api/quotations/{id}/notes` (registra history). UI `internal-notes-section` con `internal-notes-input` + `save-notes-btn` en QuotationDetail, sin abrir el modo Editar.
- **Punto 3 — Paquete privado (paridad con servicios):** `is_private` en `PackageCreate` Y `PackageUpdate` (⚠️ el retest iter_60 corrigió la omisión en PackageUpdate). Toggle `pkg-private-checkbox` en PackageEditor, insignia `package-private-badge` en Packages, exclusión del catálogo/detalle público (`/api/public/company/{slug}`), disponible al cotizar internamente.
- Nota: en este entorno la IA BYOK no está configurada → follow-up-* responden 503 (esperado). El endpoint público del catálogo es `/api/public/company/{slug}`.


## Iteración 4 / iter_58 (jun-2026) — Flujo de pago, comisión bancaria, estados (COMPLETADO ✅)
**Incluye fix del P0 de DOBLE COBRO real en Stripe.** Testing: `/app/test_reports/iteration_58.json` — backend 13/13 PASS (`tests/test_iteration58_payment_config.py`) + frontend 100%. NO se tocó `pricing.py` ni Paquete armado.
- **P0-A/P0-B anti-doble-cobro (reconciliación):** causa raíz = `emergentintegrations` no verifica firma sin `webhook_secret` ni registra el endpoint en Stripe → entrega de webhook poco fiable → pago invisible (amount_paid=0) → el guard `amount_due<=0` nunca se activaba. Fix: `_reconcile_quotation_payments(db, quotation_id, company)` en `deps.py` consulta a Stripe SOLO las txns NO finalizadas de la cotización (idempotente vía `find_one_and_update({payment_status:{$ne:'paid'}})`) y aplica pagos detectados sin depender del webhook. Se dispara en `get_public_quotation`, `get_quotation` (detalle) y al inicio de `public_checkout` (antes de calcular `amount_due`). 3 capas de defensa: reconciliar en vista pública → reconciliar en checkout → guard `amount_due<=0 => 400`. Regenerar el enlace ya NO permite un 2º cobro.
- **Gating de pago por etapas (punto 1):** nuevo `PATCH /api/quotations/{id}/payment-config` (`QuotationPaymentConfig`: `payment_enabled`, `allowed_pay_type` 'deposit'|'full', `card_fee_enabled`, `card_fee_percent`) con registro en historial. `public_checkout` → 403 si no habilitado, 400 si el pay_type no coincide con el permitido. `get_public_quotation` devuelve `payment.enabled` solo si `stripe_ready AND q.payment_enabled`, más `allowed_pay_type`. UI: panel `payment-gating` en `QuotationDetail.jsx` (visible tras aceptación).
- **Comisión bancaria (punto 2):** `companies.card_fee_percent` (default 4.5, editable en Ajustes→Pagos `card-fee-percent-input`) + checkbox por reserva + % editable por reserva. Solo tarjeta/Stripe (nunca transferencia). La txn guarda `amount` (reserva+comisión, lo cobrado) y `base_amount` (solo reserva); `_apply_payment_to_quotation` acredita SOLO `base_amount` → la comisión no infla `amount_paid`. Desglose visible en el enlace (`card-fee-breakdown`).
- **Sello de pago en PDF de Confirmación (punto 3):** `BookingConfirmationSave.payment_stamp` ('auto'|'paid'|'pending'). El PDF muestra sello **PAGADO** (verde) / **PENDIENTE DE PAGO** (ámbar). 'auto' deriva de `quotation.payment_status`; el ejecutivo puede forzarlo (`payment-stamp-select`). NO se creó recibo aparte; los 2 formatos de PDF existentes intactos, solo se AGREGÓ el sello.
- **Estados (punto 4):** "Ganada" → **"Aceptada" SOLO en UI** (QuotationDetail, Kanban, QuotationsList, Dashboard, AuditLog). El valor interno sigue `ganada` (Literal sin cambios, cero migración). "Pagada" = **insignia dorada** (amber) ligada a `payment_status`, no columna → una cotización puede estar Aceptada + Pagada a la vez.
- **Enlace del cliente (punto 5):** `PublicQuotation.jsx` muestra SOLO el tipo permitido (`pay-total-btn` o `pay-deposit-btn`) y solo cuando el pago está habilitado; jerarquía aceptar → pagar.
- **Cliente + ejecutivo en nueva cotización (punto 6):** buscador `client-search-input` (nombre/empresa/correo) y nuevo paso `executive` en el wizard (`step-executive-panel`) que aparece solo para clientes con ejecutivos; cliente directo lo omite.
- Nota (pre-existente, NO de esta iter): hydration warning `<span> child of <option>` en EmailSettings.jsx por inyección de plataforma; no bloquea nada.
...
- **Cierre (iter_57):** advertencia consciente al **descargar PDF** y al **enviar** (WhatsApp/correo) con desfase. `guardMismatch(actionLabel)` reutiliza el array `mismatches` (comparación `_expected`) y muestra `ConfirmDialog` (`confirm-dialog`): "…de todos modos" (continúa, NO bloquea) / "Actualizar primero" (dispara `refresh-amounts` y aborta el envío). Sin desfase no aparece. Tests: `/app/test_reports/iteration_57.json` — frontend 6/6 PASS.
...
## [2026-07-16] Lote D — Cliente favorito + Itinerario según tipo (COMPLETADO ✅, iter_50)
- 4.1 Cliente favorito: estrella manual (ClientUpdate.is_favorite) en /app/clients y en los pickers de Paquete y Personalizado. Orden automático: favoritos manuales primero, luego por frecuencia (my_freq = # de cotizaciones elaboradas por el ejecutivo logueado con ese cliente, vía created_by), luego por nombre. list_clients expone is_favorite y my_freq. Helper /app/frontend/src/lib/clients.js (sortByFavorite). Sort 'favorites' por defecto en Clients.jsx.
...
## [2026-07-21] Fix Turnstile token de un solo uso + doc de variable (iter en curso)
- Captcha RESUELTO en producción: la causa era la falta de `REACT_APP_TURNSTILE_SITE_KEY` en el build del frontend (no era Cloudflare). Log confirma "Turnstile OK hostname=routiq.com.mx".
- **VARIABLE DE DESPLIEGUE REQUERIDA**: `REACT_APP_TURNSTILE_SITE_KEY` (frontend, build-time) y `TURNSTILE_SECRET_KEY` (backend). Sin la site key el signup queda sin captcha; Signup.jsx muestra un aviso ámbar visible (data-testid='signup-turnstile-missing') para que un build mal configurado no falle en silencio.
- Fix token de un solo uso (Signup.jsx): tras CUALQUIER error del backend se resetea el widget (`turnstileRef.current.reset()`) y se limpia el token, para que el reintento use un token nuevo (evita el falso 'timeout-or-duplicate'). Backend `_verify_turnstile` ahora devuelve (ok, code); el endpoint /signup mapea timeout-or-duplicate/invalid-input-response/missing-input-response → 400 "La verificación expiró, inténtalo de nuevo." [marcador '[turnstile:timeout-or-duplicate]']; otros códigos → mensaje genérico. Verificado iter_55: backend 22/22 pytest + frontend Playwright, sin issues.
- Backlog P2 (fuera de esta iteración): miniaturas WhatsApp, dry-run Excel, pago por ocupación, top agencias/ejecutivos, timeline público, comparar dos meses. Refactor pendiente (aislado, post-deploy Iter3): extraer "Conceptos adicionales" a componente reutilizable (refactor puro + regresión de precios).
