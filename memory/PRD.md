...
### P0 — siguientes
- [x] **Control de planes por empresa (Master)** ✅ (jun-2026, iter_13): el Super Admin asigna plan (Starter/Pro/Enterprise) por tenant desde `/master/companies` → botón "Plan" → modal con presets que aplican límite de ejecutivos + toggles (IA, Stripe, Transferencia, White-label). `PATCH /api/companies/{id}/plan` (PLAN_DEFAULTS en server.py). El Tenant respeta el límite: banner de uso "X de Y ejecutivos" en `/app/team`, botón "Invitar" deshabilitado y aviso de upgrade al alcanzarlo; backend devuelve 403 al exceder (`POST /users/invite-executive`).
