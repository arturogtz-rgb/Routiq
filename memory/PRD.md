...
### P0 — siguientes
- [x] **Seguridad del funnel + historial de solicitudes (Master)** ✅ (jun-2026, iter_15): `POST /api/signup` protegido con **rate-limit por IP** (5/hora, 20/día vía X-Forwarded-For, colección `signup_attempts` con índice TTL de 24h), **honeypot** (campo oculto `website`) y **Cloudflare Turnstile** listo para enchufar (verificación server-side se salta mientras `TURNSTILE_SECRET_KEY` esté vacío; widget condicional con `REACT_APP_TURNSTILE_SITE_KEY`; paquete `@marsidev/react-turnstile`). En `/master/companies` se añadió **Historial de solicitudes** con filtro (Todas/Aprobadas/Rechazadas), estado, fecha de decisión y motivo de rechazo. Pendiente: el usuario enchufará las llaves de Turnstile cuando las obtenga de Cloudflare.
