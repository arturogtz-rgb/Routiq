...
import {
  MessageCircle, Kanban, FileText, Calculator, Sparkles, Smartphone, ArrowRight, Check,
} from 'lucide-react';
...
const FALLBACK = {
  ...
  hero_subtitle: 'Routiq convierte tus chats de WhatsApp en cotizaciones estructuradas, con pipeline visual, PDF profesional y un motor de precios 100% configurable.',
...
function Hero({ c }) {
  return (
    <section className="relative overflow-hidden">
      {c.hero_image_url && <img src={c.hero_image_url} alt="" className="absolute inset-0 w-full h-full object-cover opacity-30" data-testid="landing-hero-image" />}
      ...
      <div className="relative max-w-7xl mx-auto px-4 md:px-8 py-20 md:py-28 grid md:grid-cols-12 gap-10 items-center">
        <div className="md:col-span-7 space-y-6 animate-fade-up">
          <span className="pill bg-brand-50 text-brand-500" data-testid="hero-pill">
            <Sparkles className="w-3.5 h-3.5" /> {c.hero_pill}
          </span>
          <h1 className="font-display text-4xl sm:text-5xl lg:text-[64px] leading-[1.02] font-bold text-ink-900 tracking-tight" data-testid="landing-hero-title">
            {c.hero_title} <br />
            <span className="text-brand-500">{c.hero_highlight}</span>
          </h1>
          <p className="text-lg text-ink-500 max-w-xl leading-relaxed" data-testid="landing-hero-subtitle">{c.hero_subtitle}</p>
          <div className="flex flex-wrap gap-3 pt-2">
            <Link to="/login" className="btn-primary" data-testid="hero-cta-primary">
              {c.cta_primary} <ArrowRight className="w-4 h-4" />
            </Link>
            <a href="#features" className="btn-secondary" data-testid="hero-cta-secondary">{c.cta_secondary}</a>
          </div>
          <div className="flex items-center gap-4 pt-4 text-sm text-ink-500">
            <div className="flex -space-x-2">
              {['#185FA5', '#378ADD', '#E1F5EE', '#FAEEDA'].map((col, i) => (
                <div key={i} className="w-8 h-8 rounded-full border-2 border-white" style={{ background: col }} />
              ))}
            </div>
            <span><b className="text-ink-900">{c.waitlist_text}</b></span>
          </div>
        </div>
        <div className="md:col-span-5"><MockDashboardCard /></div>
      </div>
    </section>
  );
}
      ...
      <div className="relative card-surface p-5 md:p-6 space-y-4 animate-fade-up" style={{ animationDelay: '0.15s' }}>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={card.label} className="rounded-xl border border-ink-100 p-3">
              <p className="text-[11px] uppercase font-bold tracking-wider text-ink-400">{card.label}</p>
              <p className="font-display font-bold text-2xl text-ink-900">{card.n}</p>
              <span className={`pill mt-2 ${card.tone}`}>activas</span>
            </div>
          ))}
        </div>
...
function Features({ c }) {
  const items = [
    ...
    { icon: Sparkles, title: 'IA operativa', text: 'Resúmenes automáticos del chat, detección de oportunidades y campos faltantes.' },
    ...
    <section id="features" className="py-20 md:py-28">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="max-w-2xl">
          ...
          <h2 className="font-display text-3xl md:text-5xl font-semibold text-ink-900 mt-4 tracking-tight" data-testid="landing-features-title">{c.features_title}</h2>
          <p className="text-ink-500 mt-4 text-lg">{c.features_subtitle}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-10">
          {items.map(({ icon: Icon, title, text }, i) => (
            <div key={title} className="card-surface p-6 animate-fade-up" style={{ animationDelay: `${i * 0.05}s` }}>
              <div className="w-11 h-11 rounded-xl bg-brand-50 text-brand-500 flex items-center justify-center mb-4"><Icon className="w-5 h-5" /></div>
    ...
    <section id="how" className="py-20 md:py-28 bg-white border-y border-ink-100">
      <div className="max-w-7xl mx-auto px-4 md:px-8 grid md:grid-cols-12 gap-10 items-start">
        <div className="md:col-span-5">
          ...
          <h2 className="font-display text-3xl md:text-4xl font-semibold text-ink-900 mt-4 tracking-tight">Del caos de WhatsApp a un flujo que tu equipo sigue.</h2>
          <p className="text-ink-500 mt-4 leading-relaxed">Arranca en un día. Sin romper lo que ya funciona en Fareharbor ni Bokun. Compatible con Meta API oficial cuando tu empresa esté lista.</p>
          {c.feature_image_url && <img src={c.feature_image_url} alt="Equipo de ventas" className="rounded-2xl mt-8 hidden md:block shadow-lg" data-testid="landing-feature-image" />}
        </div>
  ...
  return (
    <section id="demo" className="relative py-24 md:py-32 bg-brand-500 text-white overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto px-4 md:px-8 text-center space-y-6">
        <h2 className="font-display text-4xl md:text-6xl font-bold tracking-tight" data-testid="landing-finalcta-title">{c.final_cta_title}</h2>
        <p className="text-lg text-brand-50 max-w-2xl mx-auto">{c.final_cta_subtitle}</p>
